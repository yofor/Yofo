...
function applyOrderTransition(order, toState, actorId, reason) {
  const event = logOrderEvent(order, toState, actorId, reason);
  Object.assign(order, transitionOrder(order, toState));
  return event;
}
  ...
  return http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === "POST" && path === "/listings") {
        ...
        if (!body.proof_video_url || !body.proof_dynamic_code) {
          return badRequest(res, "proof_video_url and proof_dynamic_code are required for trust verification");
        }
      ...
      if (req.method === "GET" && path.match(/^\/orders\/[^/]+\/events$/)) {
        const orderId = path.split("/")[2];
        const events = db.order_events.filter((item) => item.order_id === orderId);
      ...
      if (req.method === "POST" && path.match(/^\/orders\/[^/]+\/pay$/)) {
        ...
        applyOrderTransition(order, ORDER_STATES.PAID_ESCROW, "buyer", "payment held in escrow");
      ...
      if (req.method === "POST" && path.match(/^\/orders\/[^/]+\/commit$/)) {
        ...
        const order = db.orders.find((item) => item.id === orderId);
        if (!order) return notFound(res);
        applyOrderTransition(order, ORDER_STATES.SELLER_COMMITTED, "seller", "seller accepted delivery commitment");
        return sendJson(res, 200, order);
      }
      ...
      if (req.method === "POST" && path.match(/^\/orders\/[^/]+\/delivery-event$/)) {
        ...
        if (order.state === ORDER_STATES.SELLER_COMMITTED) {
          applyOrderTransition(order, ORDER_STATES.IN_TRANSIT, body.actor_id || "rider", "delivery started");
        }
      ...
      if (req.method === "POST" && path.match(/^\/orders\/[^/]+\/confirm$/)) {
        ...
        if (order.state === ORDER_STATES.IN_TRANSIT) {
          applyOrderTransition(order, ORDER_STATES.DELIVERED, "buyer", "buyer marked delivered");
        }
        applyOrderTransition(order, ORDER_STATES.COMPLETED, "system", "escrow released to seller");
        return sendJson(res, 200, order);
      }
      ...
      if (req.method === "POST" && path.match(/^\/orders\/[^/]+\/dispute$/)) {
        ...
        const order = db.orders.find((item) => item.id === orderId);
        if (!order) return notFound(res);
        applyOrderTransition(order, ORDER_STATES.DISPUTED, "buyer", "buyer opened dispute");
        return sendJson(res, 200, order);
      }

      if (req.method === "GET" && path.match(/^\/sellers\/[^/]+\/score$/)) {
        const sellerId = path.split("/")[2];
        return sendJson(res, 200, getSellerScore(sellerId));
      }
